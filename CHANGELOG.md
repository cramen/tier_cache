...
- Tag indexes are now bounded: per-tag member sets and the per-key reverse index are written with the data entry's physical TTL, so they expire with the data instead of growing without bound as entries expire naturally; `keysByTag` also filters and prunes members whose data key is gone, so tag lookups never return phantom entries.
- Reconnect recovery no longer flushes L1 when nothing was lost: a from-the-beginning journal cursor (`0-0`, recorded against an empty journal) is never treated as trimmed, and the replay cursor now advances as live invalidations are consumed (bounded cadence, not a per-message round trip). Previously a short reconnect after normal operation could force a full L1 flush — a surprise cold cache and a load spike.

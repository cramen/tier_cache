...
class RedisNamespaceWiringTest {
    @Test void v2NamespacesAndReplayWorkThroughTwoMicronautContexts() throws Exception {
        for (String image : new String[]{"redis:6.2-alpine", "valkey/valkey:8.0-alpine"}) {
            try (var server = new GenericContainer<>(DockerImageName.parse(image)).withExposedPorts(6379)) {
                ...
                Map<String, Object> config = Map.of("tiercache.enabled", "true", "tiercache.redis-uri", uri);
                try (var a = ApplicationContext.run(config); var b = ApplicationContext.run(config)) {
                    verify(a.getBean(TierCacheFactory.class), b.getBean(TierCacheFactory.class),
                            a.getBean(RedisStreamJournal.class), a.getBean(io.lettuce.core.RedisClient.class), "micronaut:");
                }
            }
        }
    }
